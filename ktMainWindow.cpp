...
wxBEGIN_EVENT_TABLE(ktMainWindow, wxFrame)
    //~ EVT_MENU(ID_Hello,   ktMainWindow::OnHello)
    EVT_MENU(wxID_EXIT,  ktMainWindow::OnExit)
    EVT_MENU(wxID_ABOUT, ktMainWindow::OnAbout)
wxEND_EVENT_TABLE()

ktMainWindow::ktMainWindow(const wxString& title,const wxPoint& pos,const wxSize& size)
  ...
  CreateStatusBar();
  SetStatusText( "Welcome to kan Tools!" );
}
void ktMainWindow::OnExit(wxCommandEvent& event) {
    Close(true);
}
void ktMainWindow::OnAbout(wxCommandEvent& event) {
    wxMessageBox("This is a wxWidgets' Hello world sample",
                  "About Hello World", wxOK | wxICON_INFORMATION );
}

//~ void ktMainWindow::OnHello(wxCommandEvent& event)
//~ {
